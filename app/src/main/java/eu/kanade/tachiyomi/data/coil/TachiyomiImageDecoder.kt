...
/**
 * FINAL FIX DECODER:
 * 1. Force ORIGINAL Resolution (Fix Blur pada Text bahkan tanpa zoom).
 * 2. Smart Hardware Bitmap (Hemat RAM & Anti Lag).
 * 3. Safe Threshold (Anti Black Screen di semua device).
 */
class TachiyomiImageDecoder(private val resources: ImageSource, private val options: Options) : Decoder {
    ...
    override suspend fun decode(): DecodeResult {
        ...
        val hardwareLimit = ImageUtil.hardwareBitmapThreshold
        
        var sampleSize = 1

        if (srcWidth > hardwareLimit || srcHeight > hardwareLimit) {
            var w = srcWidth
            var h = srcHeight
            while (w > hardwareLimit || h > hardwareLimit) {
                sampleSize *= 2
                w /= 2
                h /= 2
            }
